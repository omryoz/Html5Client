            ...
            if(browserSupported()) {
                ...
                i18n.init({ fallbackLng: 'en', postProcess: 'sprintf', resGetPath: '${cp}/i18n/__lng__.json' }, function(){
                    ...
                    Poker.AppCtx.wire({
                        ...
                        playerApiBaseUrl : "http://[DOMAIN_NAME]:8080/famews-1.0.0/gameRestService",
    ...
    <div class="view-container slidable">
        ...
        <div id="loginView" class="loading-view">
            <div class="loading-view-container">
			    ...
			    <div id="fbarea" class="login-button">
...
//     document.getElementById('status').innerHTML =
 //       'Thanks for logging in, ' + response.email + '!';
              var legalAgeRequiredMet=$("#ageAgreement").is(":checked");
        //       alert(legalAgeRequiredMet);

 //               $.ga._trackEvent("user_navigation", "use_login_button", usr);
   //             Poker.AppCtx.getCommunicationManager().doLogin(response.name,response.id,legalAgeRequiredMet,5);
     
     
	  FB.api("/me/picture?width=180&height=180",  function(response2) {

		var profileImage = response2.data.url;
	 //               $.ga._trackEvent("user_navigation", "use_login_button", usr);
//	 alert("auth " + response.name + " avatar " + profileImage);
//Temporary force age verif to true
			Poker.AppCtx.getCommunicationManager().doLogin(response.name,response.id, profileImage,true,5);
